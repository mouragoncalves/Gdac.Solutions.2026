...
public class ExceptionHandlingMiddleware(RequestDelegate next, ILogger<ExceptionHandlingMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (ValidationException ex)
        {
            ...
            await WriteProblemAsync(context, 400, "Dados inválidos.", errors: errors);
        }
        catch (UnauthorizedException ex)
        {
            await WriteProblemAsync(context, 401, ex.Message);
        }
        catch (NotFoundException ex)
        {
            await WriteProblemAsync(context, 404, ex.Message);
        }
        catch (DomainException ex)
        {
            await WriteProblemAsync(context, 400, ex.Message);
        }
        catch (Exception ex)
        {
    ...
    private static async Task WriteProblemAsync(
        HttpContext context, int status, string detail,
