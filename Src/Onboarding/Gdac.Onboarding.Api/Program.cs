using Gdac.Onboarding.Api.Extensions;
using Gdac.Onboarding.Api.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((ctx, services, cfg) =>
    {
        cfg.ReadFrom.Configuration(ctx.Configuration)
           ...
           .Enrich.WithProperty("Application", "Gdac.Onboarding");
        ...
        if (!string.IsNullOrEmpty(host) && !string.IsNullOrEmpty(alertTo))
        {
            ...
            var subject = ctx.HostingEnvironment.IsProduction()
                ? "[URGENTE] [Production] [GDAC Onboarding] Erro crítico"
                : $"[{env}] [GDAC Onboarding] Erro crítico";
    ...
    {
        app.UseSwagger();
        app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Gdac.Onboarding v1"));
    }
