using Gdac.Content.Api.Extensions;
using Gdac.Content.Api.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((ctx, services, cfg) =>
    {
        cfg.ReadFrom.Configuration(ctx.Configuration)
           ...
           .Enrich.WithProperty("Application", "Gdac.Content");
        ...
        if (!string.IsNullOrEmpty(host) && !string.IsNullOrEmpty(alertTo))
        {
            ...
            var subject = ctx.HostingEnvironment.IsProduction()
                ? "[URGENTE] [Production] [GDAC Content] Erro crítico"
                : $"[{env}] [GDAC Content] Erro crítico";
    ...
    {
        app.UseSwagger();
        app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Gdac.Content v1"));
    }
