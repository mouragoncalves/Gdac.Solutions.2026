using Gdac.Content.Domain.Entities;
using Gdac.Content.Domain.Exceptions;
using Gdac.Content.Domain.Interfaces.Repositories;
using MediatR;

namespace Gdac.Content.Application.Features.Showcase.Commands.SetShowcaseItemActive;

public class SetShowcaseItemActiveHandler(IShowcaseItemRepository repo, IUnitOfWork uow)
    : IRequestHandler<SetShowcaseItemActiveCommand>
{
    public async Task Handle(SetShowcaseItemActiveCommand request, CancellationToken ct)
    {
        var item = await repo.GetByIdAsync(request.Id, ct)
            ?? throw new NotFoundException(nameof(ShowcaseItem), request.Id);

        item.SetActive(request.IsActive);
        repo.Update(item);
        await uow.CommitAsync(ct);
    }
}
