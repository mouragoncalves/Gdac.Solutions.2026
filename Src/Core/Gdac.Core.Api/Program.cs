using Gdac.Core.Api.Extensions;
using Gdac.Core.Api.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((ctx, services, cfg) =>
    {
        ...
        if (!string.IsNullOrEmpty(host) && !string.IsNullOrEmpty(alertTo) && !string.IsNullOrEmpty(password))
        {
            ...
            var subject = ctx.HostingEnvironment.IsProduction()
                ? "[URGENTE] [Production] [GDAC Core] Erro crítico"
                : $"[{env}] [GDAC Core] Erro crítico";
    ...
    {
        app.UseSwagger();
        app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Gdac.Core v1"));
    }
